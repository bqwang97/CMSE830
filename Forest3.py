...
with tab2:
    ...
    with tab9:
        ...
        with col2:
            ...
            st.pyplot(fig)
        st.markdown('<p class="font_text"> From the histplot, it is obvious to see that the log change reduces skewness and improve symmetry </p>', unsafe_allow_html=True)
    with tab10:
...
with tab5:
    ...
    st.markdown('<p class="font_text"> The SVM model shows a better prediction for small burned area region. </p>', unsafe_allow_html=True)
#########################################################################################################################################
with tab6:
    st.markdown('<p class="font_header"> Cross Validation Analysis: </p>', unsafe_allow_html=True)
    ...
    means = [linear_mean,svm_mean, rf_mean]
    stds = [linear_std,svm_std, rf_std]
    ...
    st.markdown('<p class="font_text"> The SVM model seems to perform best among the three as it has the least negative score (the bar is closest to zero). The Random Forest model is the second-best, and the Linear model is the worst-performing in terms of the cross-validation score used. </p>', unsafe_allow_html=True)

    st.markdown('<p class="font_header"> Regression Error Characteristic:</p>', unsafe_allow_html=True)
    def calculate_rec_curve(y_true, y_pred, thresholds):
        """Calculate the REC curve values."""
        errors = np.abs(y_true - y_pred)
        proportions = [np.mean(errors <= thresh) for thresh in thresholds]
        return proportions
    
    max_error = st.number_input('Maximum Error for REC Curve', min_value=0.0, max_value=5.0, value=1.0)
    thresholds = np.linspace(0, max_error, 100)
    # Calculate REC curve values for each model
    rec_linear = calculate_rec_curve(y_test, lin_reg_predictions, thresholds)
    rec_rf = calculate_rec_curve(y_rf_test, rf_reg_predictions, thresholds)
    rec_svm= calculate_rec_curve(y_svm_test, SVM_reg_predictions, thresholds)

    # Plotting the REC curves
    fig5 = go.Figure()
    fig5.add_trace(go.Scatter(x=thresholds, y=rec_linear, mode='lines', name='Linear'))
    fig5.add_trace(go.Scatter(x=thresholds, y=rec_rf, mode='lines', name='RF'))
    fig5.add_trace(go.Scatter(x=thresholds, y=rec_svm, mode='lines', name='SVM'))
    fig5.update_layout(title='REC Curves',
                      xaxis_title='Admissible Absolute Error',
                      yaxis_title='Proportion of Data Points',
                      template="plotly_white")

    # Display the Plotly graph in Streamlit
    st.plotly_chart(fig5)
    st.markdown('<p class="font_text"> The plot compares three models: Linear Regression (Linear), Random Forest (RF), and Support Vector Machine (SVM). Each line represents the performance of one model in terms of the proportion of predictions that fall within a given error threshold. </p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> At lower thresholds (closer to 0), a smaller proportion of predictions are within the error tolerance for all models. As the threshold increases, a higher proportion of predictions fall within the error tolerance. </p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> The SVM model appears to perform the best across all thresholds, as indicated by the highest line on the plot. This suggests that the SVM model has the highest proportion of predictions within a given error tolerance, making it the most accurate model among the three. </p>', unsafe_allow_html=True)
#########################################################################################################################################
with tab7:
    st.markdown('<p class="font_header">Summary </p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> The database included spatial, temporal, components from the Canadian Fire Weather Index (FWI) and four weather conditions. This problem was modeled as a regression task, where the aim was the prediction of the burned area. Three different machine learning model and four feature selections (using distinct combinations of FWI elements and meteorological variables) were tested.</p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> SVM model based on weather inputs is capable of predicting small fires, which constitute the majority of the fire occurrences. The drawback is the lower predictive accuracy for large fires.</p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> To improve it, we believe that additional information (not available in this study) is required, such as the type of vegetation and firefighting intervention (e.g. time elapsed and firefighting strategy). Nevertheless, the proposed model is still useful to improve firefighting resource management.</p>', unsafe_allow_html=True)

    st.subheader('About the developer')
    st.markdown('<p class="font_text"> Hello! I am Bingqing Wang,the developer of this web application. I am a second-year Ph.D. student from department of Electrical and Computer Engineering. I develped this project to help people understand the influnce of forestfires better and explore the application of machine learning tool in predicting and reduce the damage of forest fires.</p>', unsafe_allow_html=True)
    st.markdown('<p class="font_text"> Thanks for your attention!</p>', unsafe_allow_html=True)
#########################################################################################################################################
#Reference
